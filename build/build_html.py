...
CSS = """
:root{
  --paper:#F4EEE2;--paper-2:#FBF7EF;--card:#FCFAF4;--ink:#221C17;--ink-soft:#5C534A;--ink-faint:#8C8273;
...
body{margin:0;background:var(--paper);color:var(--ink);font-family:"IBM Plex Sans",system-ui,sans-serif;
  font-size:17px;line-height:1.62;letter-spacing:.005em;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility;position:relative}
...
const io=new IntersectionObserver((ents)=>{ents.forEach(en=>{if(en.isIntersecting){const id=en.target.id.replace('fig_','');en.target.classList.add('in');
  if(BUILD[id]&&!done[id]){done[id]=true;try{BUILD[id]();}catch(e){console.error(id,e);}} if(id==='nom')wireWest(); io.unobserve(en.target);}});},{threshold:.12});
